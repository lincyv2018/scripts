...
f = open("uploadfilelist"+str(datetime.datetime.today())+".log", "a+") 
...
def __deploy_asset_to_s3(data, path, size, bucket, compress=True):
    ...
    try:
        ...
        key.set_contents_from_string(data, headers=headers, replace=True, reduced_redundancy=False)
...
def deploy_tarball_to_s3(tarball_obj, bucket_name, prefix='', region='eu-west-1', concurrency=100, no_compress=False, strip_components=0):
    """
        ...
        with tarfile.open(name=None, mode="r:*", fileobj=tarball_obj) as tarball:
            ...
            try:
                for member in tarball:
                    ...
                    pool.wait_available()
                    f.write(((str(stripped_name)).strip("['")).replace("']", ','))
...
def main():
    ...
    parser.add_argument("-r", "--region", dest="region", type=str, default="eu-west-1",
                        help="Region of S3 bucket. Default=eu-west-1")
    ...
    parser.add_argument("--strip-components", dest="strip_components", type=int, default=1,
